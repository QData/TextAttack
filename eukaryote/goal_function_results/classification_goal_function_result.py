...
import eukaryote
from eukaryote.shared import utils
...
class ClassificationGoalFunctionResult(GoalFunctionResult):
    ...
    def _processed_output(self):
        ...
        if self.attacked_text.attack_attrs.get("label_names") is not None:
            ...
            output = eukaryote.shared.utils.process_label_name(output)
            color = eukaryote.shared.utils.color_from_output(output, output_label)
            return output, color
        else:
            color = eukaryote.shared.utils.color_from_label(output_label)
