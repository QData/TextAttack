...
import eukaryote
from eukaryote import DatasetArgs, ModelArgs
from eukaryote.commands import TextAttackCommand
from eukaryote.model_args import HUGGINGFACE_MODELS, TEXTATTACK_MODELS

logger = eukaryote.shared.utils.logger
...
class EvalModelCommand(TextAttackCommand):
    ...
    def get_preds(self, model, inputs):
        with torch.no_grad():
            preds = eukaryote.shared.utils.batch_model_predict(model, inputs)
        return preds
    ...
    def test_model_on_dataset(self, args):
        ...
        while i < min(args.num_examples, len(dataset)):
            ...
            for (text_input, ground_truth_output) in dataset_batch:
                attacked_text = eukaryote.shared.AttackedText(text_input)
    ...
    def run(self, args):
        ...
        eukaryote.shared.utils.set_seed(args.random_seed)
