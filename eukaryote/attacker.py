...
class Attacker:
    ...
    def _attack(self):
        ...
        while worklist:
            ...
            try:
                ...
                continue
            example = eukaryote.shared.AttackedText(example)
            ...
            if (
                ...
                new_checkpoint = eukaryote.shared.AttackCheckpoint(
                    self.attack_args,
    ...
    def attack_dataset(self):
        ...
        eukaryote.shared.utils.set_seed(self.attack_args.random_seed)
    ...
    def from_checkpoint(cls, attack, dataset, checkpoint):
        ...
        """
        assert isinstance(
            checkpoint, (str, eukaryote.shared.AttackCheckpoint)
        ), f"`checkpoint` must be of type `str` or `textattack.shared.AttackCheckpoint`, but got type `{type(checkpoint)}`."

        if isinstance(checkpoint, str):
            checkpoint = eukaryote.shared.AttackCheckpoint.load(checkpoint)
    ...
    def attack_interactive(attack):
        ...
        while True:
            ...
            example = eukaryote.shared.attacked_text.AttackedText(text)
...
def attack_from_queue(
    ...
    eukaryote.shared.utils.set_seed(attack_args.random_seed)
