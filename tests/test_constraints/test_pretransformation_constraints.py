...
class TestPretransformationConstraints:
	def test_input_column_modification_basic(self, sentence_attacked_text, entailment_attacked_text):
		constraint = textattack.constraints.pre_transformation.InputColumnModification(
			['text'], {}
		)
		assert constraint._get_modifiable_indices(sentence_attacked_text) == set(range(23))

		assert constraint._get_modifiable_indices(entailment_attacked_text) == set(range(49))

	def test_input_column_modification_premise(self, entailment_attacked_text):
		constraint = textattack.constraints.pre_transformation.InputColumnModification(
			['premise', 'hypothesis'], { 'hypothesis' } # don't modify 'hypothesis' column
		)
		assert constraint._get_modifiable_indices(entailment_attacked_text) == set(range(0, 36))
	...
	def test_input_column_modification_hypothesis(self, entailment_attacked_text):
		...
		assert constraint._get_modifiable_indices(entailment_attacked_text) == set(range(36, 49))


	def test_max_word_index(self, sentence_attacked_text):
		short_constraint = textattack.constraints.pre_transformation.MaxWordIndexModification(8)
		assert short_constraint._get_modifiable_indices(sentence_attacked_text) == set(range(8))

		long_constraint = textattack.constraints.pre_transformation.MaxWordIndexModification(8000)
		assert long_constraint._get_modifiable_indices(sentence_attacked_text) == set(range(len(sentence_attacked_text.words)))


	def test_repeat_modification(self, sentence_attacked_text, entailment_attacked_text):
		constraint = textattack.constraints.pre_transformation.RepeatModification()
		assert constraint._get_modifiable_indices(sentence_attacked_text) == set(range(len(sentence_attacked_text.words)))
		assert constraint._get_modifiable_indices(entailment_attacked_text) == set(range(len(entailment_attacked_text.words)))
	...
	def test_stopword_modification(self, sentence_attacked_text, entailment_attacked_text):
