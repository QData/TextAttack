...
TRANSFORMATION_CLASS_NAMES = {
    ...
    "word-swap-language-model": "textattack.transfomrations.WordSwapLanguageModel",
}
...
def get_args():
    ...
    parser.add_argument(
        ...
        type=str,
        required=False,
        default="bert-yelp-sentiment",
        choices=MODEL_CLASS_NAMES.keys(),
...
def parse_checkpoint_from_args(args):
    if args.checkpoint_file.lower() == "latest":
        chkpt_file_names = [
            f for f in os.listdir(args.checkpoint_dir) if f.endswith(".ta.chkpt")
        ]
