...
class TokenizedText:
    ...
    def text_window_around_index(self, index, window_size):
        ...
        text_idx_end = self._text_index_of_word_index(
            end) + len(self.words[end])
    ...
    def text_after_word_index(self, i):
        ...
        look_after_index = self._text_index_of_word_index(
            i) + len(self.words[i])
        return self.text[look_after_index:]
...
def raw_words(s):
    ...
    for c in ' '.join(s.split()):
        if c.isalpha():
            ...
            if word is not TokenizedText.SPLIT_TOKEN:
                words.append(word)
            word = ''
