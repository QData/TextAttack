...
class neuronMultiSectionCoverage(ExtrinsicCoverage):
		...
		def __init__(
				...
				self.coverage_word_dicts = torch.zeros((self.bins_word+3, 13, self.max_seq_len, 768))
				self.coverage_attention_dicts = torch.zeros((self.bins_attention + 3, 12, 12, self.max_seq_len, self.max_seq_len))
				self.min_word_coverage_tracker = torch.zeros((13, self.max_seq_len, 768)).fill_(min_value)
				self.min_attention_coverage_tracker = torch.zeros((12, 12, self.max_seq_len, self.max_seq_len)).fill_(min_attention_value)

				self.max_word_coverage_tracker = torch.zeros(( 13, self.max_seq_len, 768)).fill_(max_value)
				self.max_attention_coverage_tracker = torch.zeros(( 12, 12, self.max_seq_len, self.max_seq_len)).fill_(max_attention_value)
		...
		def _update_initial_word_coverage(self, embeddings, word_mask = None, interaction_mask = None):
				...
				self.max_word_coverage_tracker[:,indices_to_fill, :] = torch.where(torch.max(embeddings, dim = 0).values.detach() > self.max_word_coverage_tracker[:,indices_to_fill, :] , torch.max(embeddings, dim = 0).values.detach(), self.max_word_coverage_tracker[:,indices_to_fill, :])
				self.min_word_coverage_tracker[:,indices_to_fill, :] = torch.where(torch.min(embeddings, dim = 0).values.detach() \
				 < self.min_word_coverage_tracker[:,indices_to_fill, :] , torch.min(embeddings, dim = 0).values.detach(), self.min_word_coverage_tracker[:,indices_to_fill, :])
		...
		def _update_initial_coverage(self, all_hidden_states, all_attentions, word_mask = None):
		...
		def initialize_from_training_dataset(self, trainset, masks = None, bz=1):
				...
				with torch.no_grad():
					for t in tqdm(trainset):
						...
						self._update_initial_coverage(all_hidden_states, all_attentions, masks[start :end])
						start = end
		...
		def _update_word_coverage(self, all_hidden_states, word_mask = None):
				...
				a = time.time()
				section_length = (self.max_word_coverage_tracker[:, indices_to_fill , :] - self.min_word_coverage_tracker[:, indices_to_fill , :] ) / self.bins_word
				...
				temp_store_activations = torch.max( (F.one_hot(section_index, num_classes = self.bins_word + 3)).permute(0,4,1,2,3), dim = 0).values
		...
		def _update_attention_coverage(self, all_attentions, masks):
				...
				section_index = torch.where(section_index>0, section_index, torch.zeros_like(section_index) + self.bins_word + 1)
				temp_storage_activations = torch.max ((F.one_hot(section_index, num_classes = self.bins_attention + 3)).permute(0,5,1,2,3,4), dim = 0).values
		...
		def _compute_coverage(self):
				...
				neuron_attention_coverage += np.count_nonzero(self.coverage_attention_dicts.numpy())
				...
				neuron_coverage_total = neuron_word_coverage_total  + neuron_attention_coverage_total 
