...
def Greedy_WED_Weak(model):
    ...
    constraints.append(WordEmbeddingDistance(min_cos_sim=COSINE_WEAK))
...
def Greedy_WED_Med(model):
    constraints = copy.deepcopy(BASIC_CONSTRAINTS)
    constraints.append(WordEmbeddingDistance(min_cos_sim=COSINE_MED))
    goal_function = UntargetedClassification(model)
    search_method = GreedySearch()
    return Attack(goal_function, constraints, WED_transformation, search_method)
...
def Greedy_WED_Strict(model):
    constraints = copy.deepcopy(BASIC_CONSTRAINTS)
    constraints.append(WordEmbeddingDistance(min_cos_sim=COSINE_STRICT))
    goal_function = UntargetedClassification(model)
    search_method = GreedySearch()
    return Attack(goal_function, constraints, WED_transformation, search_method)
