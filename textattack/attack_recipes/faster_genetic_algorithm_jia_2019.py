...
class FasterGeneticAlgorithmJia2019(AttackRecipe):
    ...
    def build(model_wrapper):
        ...
        # also use a language model_wrapper constraint to avoid nonsensical perturbations:
        ...
        # model_wrapper by more than some threshold.
        #
        ...
        # neighborhoods and language model_wrapper scores are computed.
        # Equation (4) must be applied before the model_wrapper
        # can combine information from multiple words, but it can
        # be delayed until after processing each word independently.
        # Note that the model_wrapper itself classifies using a different
        ...
        # language model_wrapper scores at each iteration of the genetic
        ...
        #
        # Second, we use a faster language model_wrapper that allows us to query
        # longer contexts; Alzantot et al. (2018) use a slower language
        # model_wrapper and could only query it with short contexts.

        # Finally, we use the language model_wrapper constraint only
        # at test time; the model_wrapper is trained against all perturbations in N(w). This encourages the model_wrapper to be
        # robust to a larger space of perturbations, instead of
        # specializing for the particular choice of language
        # model_wrapper. See Appendix A.3 for further details. [This is a model_wrapper-specific
        # adjustment, so does not affect the attack recipe.]
        #
        # Appendix A.3:
        #
        # In Alzantot et al. (2018), the adversary applies replacements one at a
        # time, and the neighborhoods and language model_wrapper scores are computed
        ...
        # assigned by a pre-trained language model_wrapper, and the window radius W and
