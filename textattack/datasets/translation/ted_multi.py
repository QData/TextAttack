...
class TedMultiTranslationDataset(HuggingFaceNLPDataset):
    ...
    def __init__(self, source_lang='en', target_lang='de', split='test'):
        ...
        if source_lang not in language_options:
            raise ValueError(f"Source language {source_lang} invalid. Choices: {sorted(language_options)}")
        if target_lang not in language_options:
            raise ValueError(f"Target language {target_lang} invalid. Choices: {sorted(language_options)}")
        self.source_lang = source_lang
        self.target_lang = target_lang
        self.label_names = ("Translation",)
    
    def _format_raw_example(self, raw_example):
        source = raw_example['translation'][raw_example['language'] == self.source_lang][0]
        target = raw_example['translation'][raw_example['language'] == self.target_lang][0]
