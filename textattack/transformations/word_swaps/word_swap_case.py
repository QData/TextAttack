...
class WordSwapRandomCharacterCase(WordSwap):
    ...
    def _get_replacement_words(self, word):
        ...
        if self.random_one:
            ...
            rev = str.upper if word[i].islower() else str.upper
            f = random.choice([rev, lambda x: x])
            candidate_word = word[:i] + f(word[i]) + word[i + 1 :]
            candidate_words.append(candidate_word)
        else:
            for i in range(start_idx, end_idx):
                f = random.choice([str.lower, str.upper])
                rev = str.upper if word[i].islower() else str.upper
                f = random.choice([rev, lambda x: x])
                candidate_word = word[:i] + f(word[i]) + word[i + 1 :]
                candidate_words.append(candidate_word)
