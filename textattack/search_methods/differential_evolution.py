...
class DifferentialEvolution(SearchMethod):
    ...
    def __init__(self, popsize=3, maxiter=5, verbose=False, max_perturbs=1):
        """A black-box adversarial search method that uses Differential
        Evolution to find perturbations that are imperceptible but fool a
        model."""
    ...
    def perform_search(self, initial_result: GoalFunctionResult) -> GoalFunctionResult:
        ...
        bounds_and_precomputed = self.get_bounds_and_precomputed(
            initial_text, self.max_perturbs
        )
        ...
        def obj(perturbation_vector):
            ...
            cand: AttackedText = self.apply_perturbation(
                initial_text, perturbation_vector, precomputed
            )
            if (
                len(self.filter_transformations([cand], initial_text, initial_text))
                == 0
            ):
                return np.inf
        ...
        _ = differential_evolution(
            obj, bounds, disp=self.verbose, maxiter=self.maxiter, popsize=self.popsize
        )  # minimises obj
