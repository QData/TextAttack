...
def register_test(function, name=None, output_file=None, desc=None):
    ...
    tests.append(PythonFunctionTest(function, name=name, output=output, desc=desc))
...
def check_gpu_count():
    ...
    if num_gpus == 0:
        raise ValueError(
            f"detected 0 GPUs. Must run local tests with a GPU. Perhaps you need to configure CUDA?"
        )
...
register_test(
    use_embedding_augmenter,
    name="use_embedding_augmenter",
    output_file="local_tests/sample_outputs/empty_file.txt",
