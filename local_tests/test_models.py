...
class CommandLineTest(TextAttackTest):
    ...
    def execute(self):
        ...
        if isinstance(self.command, tuple):
            ...
            for i in range(len(self.command) - 1):
                if i == 0:
                    proc = subprocess.Popen(
                        shlex.split(self.command[i]), stdout=subprocess.PIPE
                    )
                else:
                    proc = subprocess.Popen(
                        shlex.split(self.command[i]),
            ...
            result = subprocess.run(
                shlex.split(self.command), stdout=subprocess.PIPE, stderr=stderr_file
            )
        stderr_file.seek(
            0
        )  # go back to beginning of file so we can read the whole thing
