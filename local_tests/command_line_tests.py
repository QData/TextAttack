...
def register_test(command, name=None, output_file=None, desc=None):
    if not os.path.exists(output_file):
        raise FileNotFoundError(
            f"Error creating test {name}: cannot find file {output_file}."
        )
    output = open(output_file).read()
    tests.append(CommandLineTest(command, name=name, output=output, desc=desc))
...
register_test(
    (
        "python -m textattack --model cnn-imdb --attack-from-file "
...
register_test(
    (
        "python -m textattack --model-from-file local_tests/sample_inputs/sst_model_and_dataset.py "
...
#
register_test(
    "python -m textattack --model bert-base-uncased-mr --recipe textfooler --num-examples 10",
...
register_test(
    ...
    desc=(
        "Runs attack using targeted classification on class 2 on BERT MNLI with"
